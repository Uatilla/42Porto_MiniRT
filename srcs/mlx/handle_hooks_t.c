...
{
	printf("3 CAMERA TRANS ON CLOSE WINDOW\n");
	if (win->camera.trans)
	{
		printf("IF TRUE\n");
		printf("MTX: %d\n", win->camera.trans->rows);
	}
	else
		printf("3 CAMERA TRANS NOT FOUND\n");

	if (win)
	{
		printf("4 CAMERA TRANS ON CLOSE WINDOW\n");
		mtx_print(win->camera.trans);
...
{
	printf(" 2 CAMERA TRANS ON RENDER WINDOW\n");
	mtx_print(data->camera.trans);
