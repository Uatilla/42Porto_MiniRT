...
int	main(void)
{
	...
	data.ray.origin = (t_point){0, 0, -3, 1};
	data.ray.direction = (t_vector){0, -sqrt(2)/2, sqrt(2)/2, 0};
	...
	if (data.first_hit)
	{
		...
		printf("reflectv: %f %f %f\n", comps.reflectv.x, comps.reflectv.y, comps.reflectv.z);
		
		color = reflected_color(&comps, &data, 1);
