...
int main(void)
{
	...
	m.specular = 0.9;
	m.shininess = 200;
	m.pattern = stripe_pattern(&(t_color){1, 1, 1, 1}, &(t_color){0, 0, 0, 1}, STR);
	...
	color = pattern_at(&m.pattern, &(t_point){2.5, 0, 0, 1}, data.world.objs, STR);
...
// 	m.specular = 0.9;
// 	m.shininess = 200;
// 	m.pattern = stripe_pattern(&(t_color){0.1, 0.25, 0.9, 1}, &(t_color){0.42, 0.81, 0.1, 1}, STR);
